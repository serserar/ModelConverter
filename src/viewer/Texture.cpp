...
void
Texture::bind (GLenum texUnit) const
{
  ...
  try
    {
      ...
      auto_ptr<ImageX> img (ImageFactory::createImage (ibuff));
...
Texture2D::Texture2D (const ImageX *img, TextureFlags flags)
{
  try
    {
      // Create texture from image buffer
...
void
Texture2D::create (const ImageX *img, TextureFlags flags)
{
  if (!img->pixels ())
...
TextureRectangle::TextureRectangle (const ImageX *img, TextureFlags flags)
  : _width (0), _height (0)
{
  try
    {
...
void
TextureRectangle::create (const ImageX *img, TextureFlags flags)
{
  if (!img->pixels ())
    throw std::runtime_error ("Invalid image data");
...
TextureCubeMap::create (const vector<ImagePtr> &faces, TextureFlags flags)
{
  ...
  // for each one
  typedef map<const ImageX*, GLenum> TexTarget;
  typedef TexTarget::value_type TexPair;
  TexTarget texImages;

  texImages.insert (TexPair (boost::get_pointer(faces[0]), GL_TEXTURE_CUBE_MAP_POSITIVE_X_ARB));
  texImages.insert (TexPair (boost::get_pointer(faces[1]), GL_TEXTURE_CUBE_MAP_NEGATIVE_X_ARB));
  texImages.insert (TexPair (boost::get_pointer(faces[2]), GL_TEXTURE_CUBE_MAP_POSITIVE_Y_ARB));
  texImages.insert (TexPair (boost::get_pointer(faces[3]), GL_TEXTURE_CUBE_MAP_NEGATIVE_Y_ARB));
  texImages.insert (TexPair (boost::get_pointer(faces[4]), GL_TEXTURE_CUBE_MAP_POSITIVE_Z_ARB));
  texImages.insert (TexPair (boost::get_pointer(faces[5]), GL_TEXTURE_CUBE_MAP_NEGATIVE_Z_ARB));
