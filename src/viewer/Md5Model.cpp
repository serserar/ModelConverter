...
// ----------------------8<--------------------------------------------------
// This has nothing to do with Md5 -- Application's stuff
...
void
Md5Skeleton::draw (const Matrix4x4f &modelView, bool labelJoints)
{
  // Draw each joint
  glPointSize (5.0f);
...
void
Md5Skeleton::setNumJoints (int numJoints)
{
...
void
Md5Skeleton::addJoint (Md5Joint_t *thisJoint)
{
...
void
Md5Mesh::setupVertexArrays (Md5Skeleton *skel)
{
	...
	if (GLEW_VERSION_2_0)
	  {
	    ...
	    glVertexAttribPointer (tangentLoc, 3, GL_FLOAT,
				   GL_FALSE, 0, &_tangentArray.front ());
	  }
	else
	  {
	    glEnableVertexAttribArrayARB (tangentLoc);
	    glVertexAttribPointerARB (tangentLoc, 3, GL_FLOAT,
				      GL_FALSE, 0, &_tangentArray.front ());
	  }
	break;
      }
...
void
Md5Mesh::renderVertexArrays () const
{
...
void
Md5Mesh::setupTexture (const Texture2D *tex, GLenum texUnit) const
{
  if (!tex)
    {
...
void
Md5Model::prepare (Md5Skeleton *skel)
{
  for (int i = 0; i < _numMeshes; ++i)
    {
      if (!_meshes[i]->hiden ())
...
void
Md5Object::prepare (bool softwareTransformation)
{
...
Md5Object::render () const
{
  glPushMatrix ();
	...
	_animatedSkeleton->draw (_modelView,
		 _renderFlags & kDrawJointLabels);
      }
