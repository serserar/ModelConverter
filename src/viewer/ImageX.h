...
public:
    ...
    : _filename (that._filename), _data (NULL), _length (that._length)
  {
    ...
    std::memcpy (_data, that._data, _length);
  }
...
private:
  ...
  ImageX (const ImageX &img);

public:
  // Constructors/destructor
  ImageX (const string &name, GLsizei w, GLsizei h, GLint numMipMaps,
...
typedef boost::shared_ptr<ImageX> ImagePtr;
...
public:
  // Public interface
  static ImageX *createImage (const ImageBuffer &ibuff)
  {
    string ext;
    ImageX *result;
